...
3. Configure the backend URL by copying `.env.example` to `.env.local`. Set
   `NEXT_PUBLIC_API_BASE_URL` to your Express backend URL, for example
   `http://localhost:5000` when running the backend locally.

4. Set up the local database API:
   ...
   The backend requires MongoDB running locally or a MongoDB Atlas connection
   in `backend/.env`. See `backend/README.md` for the complete setup.
